...
- **Build-time prerender + Dataset JSON-LD (live).** `dist/index.html` now ships the live FWI in the title and OG, a canonical tag, schema.org Dataset + Organization JSON-LD, and a `<noscript>` content block. Re-runs each deploy; the daily ingest cron also fires a Vercel Deploy Hook (when `VERCEL_DEPLOY_HOOK_URL` is set) so the baked number refreshes daily, not only on code deploys. Closed the blank-first-paint gap.
