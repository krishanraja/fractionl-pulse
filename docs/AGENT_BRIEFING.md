...
4. **Agent-native.** Public REST API, genuinely no auth on read endpoints (the documented bare curl returns HTTP 200 in production today). Two documented MCP tools (`get_fractional_working_index`, `get_fwi_weekly_brief`) plus a reference implementation; a hosted MCP server is on the roadmap. `X-FWI-Score` and `X-FWI-Label` shortcut headers for cheap polling. Built for LLM agents as first-class users, not as an afterthought.
