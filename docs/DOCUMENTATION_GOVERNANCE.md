...
**Document map updated:** 9 September 2026 against `main` at `7c124e2`, when `AGENTS.md` was added below as the coding-agent entry file carrying the canon block rendered from `krishanraja/ai-harness`. Production and the public product were not read, so the reconciliation date above stands.
...
|---|---|---|
| [`AGENTS.md`](../AGENTS.md) | Coding agents: Codex reads it natively, Claude Code and Cursor are routed to it by their own rules | Entry file. Points to `NOW.md` first, then this repository's own rules. Carries the marker-delimited canon block rendered from `krishanraja/ai-harness`; nothing between the markers is hand-maintained and the docs steward never edits it |
