...
**The Fractional Working Index (FWI)** — the only real-time composite index for the fractional executive economy. Live market intelligence built for human analysts and AI agents alike.
...
| Persona | Income / size | Why they pay |
|---------|---------------|--------------|
| **Fractional executives** ($200K+ income) | Solo operator | Time market entry, set rates, decide when to take new clients. One well-timed rate move pays for years of subscription. |
...
| **VC / PE firms** ($50M+ AUM) | Investor | Validate workforce thesis. The Form D leading indicator mirrors their own deal flow patterns. |
...
- **Public REST API**: `/current`, `/history?months=N`, `/trigger` — no auth on the read paths
- **MCP tool definition** for direct agent integration (Claude, OpenAI, custom)
...
- **Live dashboard** at `pulse.fractionl.ai` with React Query + Supabase Realtime subscriptions for guaranteed freshness
- **12-week historical backfill** for trendlines from day one
...
No auth required for the read endpoints. Build against these in 2 minutes:
...
Response includes shortcut headers `X-FWI-Score` and `X-FWI-Label` for cheap polling without parsing the body.
...
| `waitlist` | Pulse Pro early access signups (anon insert, service-role read) |
| `api_keys` | Reserved for tiered API key auth (rolled out per customer) |
