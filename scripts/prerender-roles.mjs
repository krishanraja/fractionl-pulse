...
function timingRead(r) {
  ...
  const span = `${one(r.prevDemand)} to ${one(r.demand)}`;
...
function faqFor(r, def, overall) {
  ...
  if (live) {
    ...
    let a = `Advertised demand for ${lower} roles reads ${one(r.demand)} out of 100 (${r.band}) for the seven days ending ${r.asOf}${move}`;
    if (overall && overall.score != null) {
      a += ` The overall Fractional Working Index sits at ${one(overall.score)} (${overall.band}) in the same week.`;
    }
...
function rolePage(def, r, data, site) {
  ...
  const title = live
    ? `${def.label} Demand: ${one(r.demand)} of 100 (${r.band}) | Fractional Working Index`
  ...
  const move = live && r.change7d != null
    ...
    : '';
  const desc = live
    ? `Advertised demand for ${lower} roles reads ${one(r.demand)} of 100 (${r.band}) for the week ending ${r.asOf}${move}. What that says about your rate and your timing, from the weekly Fractional Working Index.`
  ...
  const others = ROLE_DEFS.filter((d) => d.slug !== def.slug)
    .map((d) => {
      ...
      const n = o && o.demand != null ? `<span class="n">${one(o.demand)}</span>` : '';
  ...
  const readingPanel = live
    ? `<section class="panel${bandClass ? ` ${bandClass}` : ''}">
        ...
        <div class="reading"><span class="score">${one(r.demand)}</span><span class="of">of 100</span><span class="chip">${escape(r.band)}</span></div>
        <div class="meta">${[
          r.change7d != null ? `<span><strong>${escape(signed(r.change7d))}</strong> vs the previous 7 days</span>` : '',
          data && data.roleAverage != null ? `<span>Six-role average ${one(data.roleAverage)}</span>` : '',
  ...
  const marketRows = overall && overall.score != null
    ? [
        `<tr><td>Fractional Working Index (overall)</td><td class="num">${one(overall.score)}</td><td class="dim">${escape(overall.band || '')}</td></tr>`,
        overall.demand != null ? `<tr><td>Demand pillar, all roles (weight 50%)</td><td class="num">${one(overall.demand)}</td><td class="dim"></td></tr>` : '',
        overall.supply != null ? `<tr><td>Supply pillar (weight 20%)</td><td class="num">${one(overall.supply)}</td><td class="dim"></td></tr>` : '',
        overall.culture != null ? `<tr><td>Culture pillar (weight 30%)</td><td class="num">${one(overall.culture)}</td><td class="dim"></td></tr>` : '',
