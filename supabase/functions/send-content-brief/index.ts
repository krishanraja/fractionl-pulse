...
function renderHtml(bj: any, weekStart: string): string {
  const topics = (bj.rising_topics || []).slice(0, 5).map((t: any, i: number) =>
    `<tr><td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;"><strong>${i + 1}. ${t.label}</strong>${t.is_breakout ? ' 🚀' : ''}<div style="color:#64748b;font-size:13px;margin-top:2px;">${t.summary || ''}</div></td><td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;color:#6366f1;font-weight:600;">${t.radar_score}</td></tr>`).join('');
  const questions = (bj.breakout_questions || []).slice(0, 5).map((q: string) => `<li style="margin:4px 0;">${q}</li>`).join('');
  const angles = (bj.priority_angles || []).slice(0, 4).map((a: any) => `<li style="margin:6px 0;"><strong>${a.angle}</strong> <span style="color:#94a3b8;">(${a.format})</span><br/><span style="color:#64748b;font-size:13px;">${a.rationale || ''}</span></li>`).join('');
